...
class Map:
    def __init__(self, width: int = 10, high: int = 10) -> None:
        # 邊界預設皆為 10
        self.__width = width
        self.__high = high
    ...
    def generate_point(self) -> None:  # 叫 Point 產生
        ...
        self.__point.generate(bodyCoordinate)
    ...
    def generate_protal(self) -> None:  # 叫 Protal 產生
        ...
        self.__snake.set_protal(
            self.__protal.get_inter_coordinate(), self.__protal.get_exit_coordinate()
        )  # 讓 Snake 知道 Protal 的座標

        # 將 Protal 座標化
        protal = self.__protal.get_inter_coordinate()
        self.__interProtal = self.__convert_protal_to_coordinate(protal)
        self.__interProtal.append(self.__protal.get_inter_coordinate()[0])

        protal = self.__protal.get_exit_coordinate()
        self.__exitProtal = self.__convert_protal_to_coordinate(protal)
        self.__exitProtal.append(self.__protal.get_exit_coordinate()[0])

    def __convert_protal_to_coordinate(self, protal: list = None) -> list:  # 將傳送門轉換為座標系
    ...
    def display(self) -> None:
        ...
        for top in range(self.__width):
            if self.__interProtal == [top, self.__high, 1]:  # 入口protal
                print("(i)", end="")
            elif self.__exitProtal == [top, self.__high, 1]:  # 出口protal
                print("(o)", end="")
            else:
                print("___", end="")  # 牆壁
        print("__ ")  # 最上左

        for yAxis in range(self.__high - 1, -1, -1):
            # 左界線
            if self.__interProtal == [-1, yAxis, 0]:
                print("(i)", end="")
            elif self.__exitProtal == [-1, yAxis, 0]:
                print("(o)", end="")
            else:
                print(" | ", end="")
            ...
            for xAxis in range(self.__width):
                if map[xAxis][yAxis] != 0:
                    print(" ", map[xAxis][yAxis], sep="", end=" ")
                else:
                    print("   ", end="")

            # 右界線
            if self.__interProtal == [self.__width, yAxis, 2]:
                print("(i)")
            elif self.__exitProtal == [self.__width, yAxis, 2]:
                print("(o)")
            else:
                print(" | ")

        print(" ¯¯", end="")  # 最下右
        for buttom in range(self.__width):
            if self.__interProtal == [buttom, -1, 3]:  # 入口protal
                print("(i)", end="")
            elif self.__exitProtal == [buttom, -1, 3]:  # 出口protal
                print("(o)", end="")
            else:
                print("¯¯¯", end="")  # 牆壁
        print("¯¯ ")  # 最下左
        ...
        print(
            self.__protalTime, 
            self.__snake._Snake__enterProtalPlace, 
            self.__snake._Snake__exitProtalPlace
