...
# 執行相關
    ...
    def __hitting_wall(self, head: list) -> bool:   # 撞牆壁
        return (not(0 <= head[0] < self.__width)    # X軸
             or not(0 <= head[1] < self.__high))    # Y軸
    
    def __hitting_body(self, head: list) -> bool:   # 撞身體
        return (head in self.__bodyCoordinate[1: self.__long: 1])

    def move(self) -> None: #移動，內有判斷是否進傳送門、撞牆、撞身體、吃 point ，實際執行時後面需接 need_generate_point 來判定是否需要生成 point
        ...
        if self.__eat_point():
            self.__bodyCoordinate = [self.__head] + self.__bodyCoordinate[0: self.__long: 1]
            self.__head = self.__bodyCoordinate[0]
            self.__long += 1    # 加長身體
        else:
            self.__bodyCoordinate = [self.__head] + self.__bodyCoordinate[0: self.__long - 1: 1]
            self.__head = self.__bodyCoordinate[0]
...
# protal 相關
    ...
    def __convert_protal_to_coordinate(self, protal: list = None, is_enter: bool = True) -> list:    # 將傳送門轉換為座標系
        if is_enter:
    ...
    def set_protal(self, enterProtal: list = None, exitProtal: list = None) -> None:    #設定傳送門，參數為非座標
        self.__enterProtalPlace = self.__convert_protal_to_coordinate(enterProtal, is_enter = True)
        self.__exitProtalPlace = self.__convert_protal_to_coordinate(exitProtal, is_enter = False)
    ...
    def __check_body_in_protal(self) -> None: # 改變下一個在傳送門的 body index
        if self.__next_body_in_protal != -1:    # -1 為預設值，如果不等於 -1 代表已經進 protal 了
            ...
            if self.__next_body_in_protal == self.__long:   #如果下一個 body index 等於 long 代表傳完了
